...
{
	unsigned long offset = block * TFTP_BLOCK_SIZE + TftpBlockWrapOffset;
	...
	if (rc) { /* Flash is destination for this packet */
		rc = flash_write ((char *)src, (unsigned long)(load_addr+offset), len);
		if (rc) {
			flash_perror (rc);
	...
	if (NetFileXferSize < newsize)
		NetFileXferSize = newsize;
}
...
TftpHandler (unsigned char * pkt, unsigned dest, unsigned src, unsigned len)
{
	...
	case TFTP_DATA:
		if (len < 2)
			return;
		len -= 2;
		TftpBlock = ntohs(*(unsigned short *)pkt);
		...
		if (TftpState == STATE_RRQ) {
			printk ("Server did not acknowledge timeout option!\n");
		}
#endif
	...
	case TFTP_ERROR:
		//printk ("\nTFTP error: '%s' (%d)\n",pkt + 2, ntohs(*(unsigned short *)pkt));
...
TftpStart (void)
{
	if (load_file[0] == '\0') {
		...
		//printk ("*** Warning: no boot file name; using '%s'\n",tftp_filename);
	} else {
		tftp_filename = load_file;
	}

	//printk ("TFTP from server ");
	print_IPaddr (NetServerIP);
	//printk ("; our IP address is ");  
	...
	if (NetOurGatewayIP && NetOurSubnetMask) {
	    ...
	    if (OurNet != ServerNet) {
		//printk ("; sending through gateway ");
	...
	/*
	if (NetFileSize) {
		printk (" Size is 0x%x Bytes = ", NetFileSize<<9);
		print_size (NetFileSize<<9, "");
	}
	*/
