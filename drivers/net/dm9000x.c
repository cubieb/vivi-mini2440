...
int dm9000_init(struct net_device *ethdev);
int dm9000_send(struct net_device *,volatile void *, int);
int dm9000_rx(struct net_device *ethdev);
void dm9000_halt(struct net_device *ethdev);
void dm9000_reset(struct net_device *ethdev);
...
struct net_device *dm9000_Register(void)
{
    ...
    if(!ethdev)
    {
      //printk("mmalloc failed in dm9000_Register!\n");
      return NULL;
    }
  
  /*usage of clear_mem is not clear yet, so skip it now.*/
  //clear_mem((unsigned long)ethdev, sizeof(struct net_device));
  strncpy(ethdev->name, "DM9000A", 16);
  ethdev->iobase = CONFIG_DM9000_BASE;

  ethdev->init = dm9000_init;
  ethdev->send = dm9000_send;
  ethdev->recv = dm9000_rx;
  ethdev->halt = dm9000_halt;
  ethdev->reset = dm9000_reset;
...
*/
int
dm9000_init(struct net_device *ethdev)
{
	int i, oft, lnk;
	//DM9000_DBG("eth_init()\n");
	...
	for (i = 0; i < 6; i++)
	{
	    ethdev->enetaddr[i] =0x10+i;
	}
	
	/*printk("MAC: %02x:%02x:%02x:%02x:%02x:%02x\n", ethdev->enetaddr[0],
	       ethdev->enetaddr[1],ethdev->enetaddr[2],ethdev->enetaddr[3],
	       ethdev->enetaddr[4], ethdev->enetaddr[5]);
	*/
	for (i = 0, oft = 0x10; i < 6; i++, oft++)
		DM9000_iow(oft, ethdev->enetaddr[i]);
...
*/
int
dm9000_send(struct net_device *ethdev,volatile void *packet, int length)
{
	char *data_ptr;
	u32 tmplen, i;
	int tmo;
